...
(defun RUN-SIMULATION (&key terminate interruption tick)
  (loop until (check-termination-condition? terminate)
...
(defun CHECK-TERMINATION-CONDITION? (terminate &optional (apply-extra-ticks t))
  (cond ((empty-queue? *events*)
	 (cond (apply-extra-ticks
		(incf *time* 2)
		(dolist (a (list *rover* *driver* *realtime* *gds*))
		  (tick a (state a)))
		(check-termination-condition? terminate nil))
	       (t
		(format t "Terminating due to empty queue at distance ~s and time ~s~%" (rover-position *rover*) *time*)
		t)))
	(terminate
	 (let ((v (funcall terminate)))
	   (when v
	     (format t "Terminating due to termination condition with message: ~s~%" v))
	   v))
	((> *time* *maximum-time*)
	 (format t "Terminating due to maximum time~%")
	 t)
	(t
	 nil)))
