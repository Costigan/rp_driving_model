...
(define-output *DRIVE-TIME-IN-HOURS* nil)	 ; hours
...
(defun CLIP-DRIVE-TO-PSR-EDGE (target)
  (cond ((> target (* *psr-2-entry* *path-multiplier*))
	 (1+ (* *psr-2-entry* *path-multiplier*)))
	((> target (* *psr-1-entry*  *path-multiplier*))
...
(defmethod RECEIVE ((r ROVER) (msg WAKEUP-SENSORS))
  (let* ((pld (make-instance (next-sensor-payload-type r) :position (rover-position r) :time-sent *time*))
	...
	;; This causes continuous transmission
...
(defmethod RECEIVE ((r ROVER) (msg DRIVE-COMMAND))
  (let ((delta (max 0 (- (drive-command-target msg) (rover-position r)))))
    (cond ((> delta 0)
	   ...
	   ;; DEBUGGING HERE
	   ;;(format t "  sending wakeup at time ~s after driving period delta=~s speed=~s delay=~s~%" *time* delta *driving-speed* (/ delta *driving-speed*))
...
(defmethod TICK ((r ROVER) state &aux delta)
  ;; DEBUGGING HERE
  ;;(format t "TICK at ~s delta=~s state=~s pos=~s target=~s~%" *time* (- *time* (last-tick r)) state (rover-position r) (rover-target r))
  (unless (eql 0 (setq delta (- *time* (last-tick r))))
    (case state
      (:driving
       (incf (rover-driving-time r) delta)
       ;; DEBUGGING HERE
       ;;(format t "  incr pos from ~s to ~s delta=~s~%" (rover-position r) (+ (rover-position r) (* delta *driving-speed*)) delta)
       (incf (rover-position r) (* delta *driving-speed*))
       (when (>= (rover-position r) (rover-target r))
	 (set-state r :waiting)
	 ;; This causes a navcam payload to be sent whenever the rover stops (what about virtual bumper?)
	 (send r r (make-instance 'wakeup-sensors))
	 ))
      (:waiting
...
(defun RUN-MODEL ()
  (let ((*events* (make-empty-queue #'receive-time))
	(*driving-distance* (* *driving-distance* *path-multiplier*))
	(*hazards* '())
	(*bumper-interrupts* '())
	(*time* 0)
	(*rover* nil)
    ...
    (run-simulation :terminate (if (fboundp 'termination-condition)
				   ...
				   #'(lambda () 
				       (or (and (>= (rover-position *rover*) *driving-distance*) 'distance-along-path)
					   (and (>= (rover-position *rover*) *simulation-distance-cutoff*) 'distance-cutoff)
					   (and (>= *time* *simulation-time-cutoff*) 'time-cutoff))))
		    :interruption 'check-for-interruption)
    (setf *final-rover-position* (rover-position *rover*))
    (setf *final-time* *time*)
    (setf *speed-made-good* (/ (/ *final-rover-position* *path-multiplier*) *final-time*))
    (setf *speed-made-good-along-path* (/ *final-rover-position* *time*))
    ...
    (setf *drive-time-in-hours* (/ *time* 3600.0))
