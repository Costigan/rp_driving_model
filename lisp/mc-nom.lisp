...
(defun EXPLORE-LOOKAHEAD (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *lookahead-distance-day* 4.0 5.0 6.0 7.0 8.0 9.0 10.0 11. 12. 13. 14. 15.)
												(enum *lookahead-distance-night* 4.0 5.0 6.0 7.0 8.0 9.0 10.0 11. 12. 13. 14. 15.)
												(enum *hazard-eval-time* 0)
												(enum *rt-science-consultation-time* 0)
												(enum *bumper-eval-time* 0))
							 :trials n 
							 :filename "lookahead-nominal.out")
	)


(defun EXPLORE-HAZARD-TRIGGER (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *hazard-trigger-rate* 0.02 0.0175 0.015 0.0125 0.01 0.0067 0.005 0.005 0.0033 0.0025 0.002 ))
							 :trials n 
							 :filename "hazard-trigger-nominal.out")
	)

(defun EXPLORE-BUMPER-TRIGGER (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *bumper-trigger-rate* 0.02 0.0175 0.015 0.0125 0.01 0.0067 0.005 0.005 0.0033 0.0025 0.002 ))
							 :trials n 
							 :filename "bumper-trigger-nominal.out")
	)

(defun EXPLORE-HAZARD-EVAL (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *hazard-eval-time* 5 10 20 30 40 50 100 150 200 250 300 400 500))
							 :trials n 
							 :filename "hazard-response-nominal.out")
	)


(defun EXPLORE-BUMPER-EVAL (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *bumper-eval-time* 5 10 20 30 40 50 100 150 200 250 300 400 500))
							 :trials n 
							 :filename "bumper-response-nominal.out")
	)


(defun EXPLORE-DECSION-TIME-DAY (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *driver-decision-time-day* 5 10 20 30 40 50 80 100 150 200 250 300 400))
							 :trials n 
							 :filename "decision-time-day-nominal.out")
	)


(defun EXPLORE-DECSION-TIME-NIGHT (&optional (n 10))
	(run-scatter :cases '(baseline balanced balanced2 kitchen-sink lidars-galore) 
							 :specs '((enum *driver-decision-time-night* 5 10 20 30 40 50 80 100 150 200 250 300 400))
							 :trials n 
							 :filename "decision-time-night-nominal.out")
	)
